...
create procedure c_weight (in p1 bigint, in p2 bigint)
{
  ...
  return 0.5 + 
	    ...
	    where ps1.ps_creatorid = p1 and ps1.ps_replyof = ps2.ps_postid and ps2.ps_creatorid = p2) +
		 ...
		 where c1.ps_creatorid = p1 and c1.ps_replyof = c2.ps_postid and c2.ps_creatorid = p2) +
		  (select 0.5 * count (*) from post c1, post c2
		   where c1.ps_creatorid = p2 and c1.ps_replyof = c2.ps_postid and c2.ps_creatorid = p1);
}
