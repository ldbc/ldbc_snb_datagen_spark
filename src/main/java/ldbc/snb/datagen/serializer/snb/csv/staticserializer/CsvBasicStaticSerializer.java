...
public class CsvBasicStaticSerializer extends StaticSerializer<HdfsCsvWriter> implements CsvSerializer {
    ...
    public List<FileName> getFileNames() {
        return ImmutableList.of(TAG, TAG_HASTYPE_TAGCLASS, TAGCLASS, TAGCLASS_ISSUBCLASSOF_TAGCLASS, PLACE, PLACE_ISPARTOF_PLACE, ORGANISATION, ORGANISATION_ISLOCATEDIN_PLACE);
    }

    @Override
    public void writeFileHeaders() {
        writers.get(TAG).writeHeader(ImmutableList.of("id", "name", "url"));
        writers.get(TAG_HASTYPE_TAGCLASS).writeHeader(ImmutableList.of("Tag.id", "TagClass.id"));
        writers.get(TAGCLASS).writeHeader(ImmutableList.of("id", "name", "url"));
        writers.get(TAGCLASS_ISSUBCLASSOF_TAGCLASS).writeHeader(ImmutableList.of("TagClass.id", "TagClass.id"));
        ...
        writers.get(ORGANISATION_ISLOCATEDIN_PLACE).writeHeader(ImmutableList.of("Organisation.id", "Place.id"));
        writers.get(PLACE_ISPARTOF_PLACE).writeHeader(ImmutableList.of("Place.id", "Place.id"));
    }
    ...
    protected void serialize(final Place place) {
        ...
        if (place.getType() == Place.CITY || place.getType() == Place.COUNTRY) {
            writers.get(PLACE_ISPARTOF_PLACE).writeEntry(ImmutableList.of(
                Integer.toString(place.getId()),
    ...
    protected void serialize(final Organisation organisation) {
        ...
        writers.get(ORGANISATION_ISLOCATEDIN_PLACE).writeEntry(ImmutableList.of(
            Long.toString(organisation.id),
    ...
    protected void serialize(final TagClass tagClass) {
        ...
        if (tagClass.parent != -1) {
            writers.get(TAGCLASS_ISSUBCLASSOF_TAGCLASS).writeEntry(ImmutableList.of(
                Integer.toString(tagClass.id),
    ...
    protected void serialize(final Tag tag) {
        ...
        writers.get(TAG_HASTYPE_TAGCLASS).writeEntry(ImmutableList.of(Integer.toString(tag.id), Integer.toString(tag.tagClass)));
    }
