...
public class Turtle implements Serializer {
    ...
    public Turtle(String file, int reducerId, int nrOfOutputFiles, boolean isTurtle,
                  ...
                  IPAddressDictionary ipDic, PlaceDictionary locationDic,
    ...
    public void serialize(UserInfo info) {
        ...
        createTripleSPO(result, prefix, SNVOC.locatedIn, DBP.fullPrefixed(locationDic.getPlaceName(info.extraInfo.getLocationId())));
    ...
    public void serialize(Post post) {
        ...
        if (post.getIpAddress() != null) {
            createTripleSPO(result, prefix, SNVOC.locatedIn,
                    DBP.fullPrefixed(locationDic.getPlaceName((ipDic.getLocation(post.getIpAddress())))));
        }
    ...
    public void serialize(Photo photo) {
        ...
        createTripleSPO(result, prefix, SNVOC.locatedIn,
                DBP.fullPrefixed(locationDic.getPlaceName((ipDic.getLocation(photo.getIpAddress())))));
    ...
    public void serialize(Comment comment) {
        ...
        if (comment.getIpAddress() != null) {
            createTripleSPO(result, prefix, SNVOC.locatedIn,
                    DBP.fullPrefixed(locationDic.getPlaceName((ipDic.getLocation(comment.getIpAddress())))));
        }
    ...
    public void serialize(Forum forum) {
        ...
        String forumPrefix = SN.getForumURI(forum.getForumId());
        AddTriple(result, true, false, forumPrefix, RDF.type, SNVOC.Forum);

        AddTriple(result, false, false, forumPrefix, SNVOC.id,
                createLiteral(SN.formId(forum.getForumId())));

        AddTriple(result, false, false, forumPrefix, SNVOC.title, createLiteral(forum.getForumName()));
        AddTriple(result, false, true, forumPrefix, SNVOC.creationDate,
                createDataTypeLiteral(dateString, XSD.DateTime));

        createTripleSPO(result, forumPrefix,
                SNVOC.hasModerator, SN.getPersonURI(forum.getModeratorId()));
    ...
    public void serialize(Place place) {
        ...
        createTripleSPO(result, DBP.fullPrefixed(name), SNVOC.id, createLiteral(Long.toString(place.getId())));
        if (place.getType() != Place.CONTINENT) {
            String countryName = locationDic.getPlaceName(locationDic.belongsTo(place.getId()));
