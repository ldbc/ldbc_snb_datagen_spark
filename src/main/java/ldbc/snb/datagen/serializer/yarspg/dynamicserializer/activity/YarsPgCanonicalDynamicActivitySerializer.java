...
public class YarsPgCanonicalDynamicActivitySerializer extends DynamicActivitySerializer<HdfsYarsPgWriter> implements YarsPgSerializer {
    ...
    public void serialize(final Post post) {
        ...
        String personSchemaEdgeID = Statement.generateId("S_Person" + post.getAuthor().getAccountId());
        String personEdgeID = Statement.generateId("Person" + post.getAuthor().getAccountId());
        writers.get(SOCIAL_NETWORK_ACTIVITY)
                .writeEdge(EdgeType.DIRECTED, (schema, edge) -> {
                    schema.as(postSchemaEdgeID, Relationship.CONTAINER_OF.toString(), personSchemaEdgeID);
                    edge.as(postEdgeID, Relationship.CONTAINER_OF.toString(), personEdgeID);
                });
    }
