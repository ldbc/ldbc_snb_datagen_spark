...
public class UpdateEventSerializer {
    ...
    private SequenceFile.Writer forumStreamWriter_[];
    private SequenceFile.Writer personStreamWriter_[];
    ...
    public UpdateEventSerializer(Configuration conf, int reducerId) {
	    conf_ = conf;
	...
	numPartitions_ = conf.getInt("ldbc.snb.datagen.serializer.numPartitions", 1);
	stats_ = new UpdateStreamStats();
        try{
            forumStreamWriter_ = new SequenceFile.Writer[numPartitions_];
            personStreamWriter_ = new SequenceFile.Writer[numPartitions_];
	    FileContext fc = FileContext.getFileContext(conf);
            for( int i = 0; i < numPartitions_; ++i ) {
                Path outFile = new Path(DatagenParams.hadoopDir + "/temp_updateStream_"+reducerId+"_"+i+"_forum");
		EnumSet<CreateFlag> createFlags = EnumSet.noneOf(CreateFlag.class);
		createFlags.add(CreateFlag.CREATE);
		createFlags.add(CreateFlag.APPEND);
                forumStreamWriter_[i] = SequenceFile.createWriter(fc, conf, outFile, LongWritable.class, Text.class, CompressionType.NONE, new DefaultCodec(),new SequenceFile.Metadata(), createFlags, Options.CreateOpts.checksumParam(Options.ChecksumOpt.createDisabled()));
                outFile = new Path(DatagenParams.hadoopDir+ "/temp_updateStream_"+reducerId+"_"+i+"_person");
                personStreamWriter_[i] = SequenceFile.createWriter(fc, conf, outFile, LongWritable.class, Text.class, CompressionType.NONE, new DefaultCodec(),new SequenceFile.Metadata(), createFlags, Options.CreateOpts.checksumParam(Options.ChecksumOpt.createDisabled()));
    ...
    public void writeKeyValue( UpdateEvent event, Stream s ) {
        try{
            ...
            switch (s) {
                case FORUM_STREAM:
                    forumStreamWriter_[nextPartition_].append(new LongWritable(event.date),new Text(string.toString()));
                    break;
                case PERSON_STREAM:
                    personStreamWriter_[nextPartition_].append(new LongWritable(event.date),new Text(string.toString()));
                    break;
            }
        } catch(IOException e){
            System.err.println(e.getMessage());
    ...
    public void close() {
	    ...
	    FileSystem fs = FileSystem.get(conf_);
            for( int i = 0; i < numPartitions_; ++i ) {
                forumStreamWriter_[i].close();
                personStreamWriter_[i].close();
		fs.delete(new Path(DatagenParams.hadoopDir + "/.temp_updateStream_"+reducerId_+"_"+i+"_person.crc"),true);
		fs.delete(new Path(DatagenParams.hadoopDir + "/.temp_updateStream_"+reducerId_+"_"+i+"_forum.crc"),true);
            }
