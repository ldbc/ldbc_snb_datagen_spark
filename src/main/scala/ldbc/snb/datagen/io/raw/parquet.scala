...
object parquet {
  ...
  private def parquetWriteSupportForEncodable[T <: Product: Encoder] = new WriteSupport[T] {
    private val encoder                                                        = encoderFor[T]
    private val serializer                                                     = encoder.createSerializer()
    private val schema                                                         = encoder.schema
    private val inner                                                          = new LdbcDatagenParquetWriteSupport(schema)
  ...
  class ParquetRecordOutputStream[T <: Product: ParquetRowEncoder](path: Path, taskAttemptContext: TaskAttemptContext) extends RecordOutputStream[T] {
    lazy val writer: RecordWriter[Void, T] = {
      implicit val encoder: Encoder[T] = implicitly[ParquetRowEncoder[T]].encoder
      val parquetWriteSupport          = parquetWriteSupportForEncodable[T]
